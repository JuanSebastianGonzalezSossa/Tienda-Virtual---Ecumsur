...
export function ShoppingReducer(state, action) {
    ...
    switch (action.type) {
        case Types.AddToCart: {
            let newItem = state.product.find(
                product => product._id === action.payload
            );

            let ItemInCar = state.cart.find(
        ...
        case Types.RemoveOneFromCart: {
            let itemDelete = state.cart.find(item => item._id === action.payload);
        ...
        case Types.ClearCart: {return shoppingInicialState}
